...
const propertyApi = {
    getAll: async (page = 1, limit = 20, search = '', status = 'all') => {
        let url = `${API_BASE}/api/properties?isAdmin=true&page=${page}&limit=${limit}`;
...
const AllPropertiesPageAdminView = () => {
    ...
    const fetchData = useCallback(async (page = 1) => {
        ...
        try {
            setLoading(true);
            setError(null);
            const data = await propertyApi.getAll(page, pagination.itemsPerPage, searchTerm, filterStatus);
    ...
    const openConfirmModal = (property, action, message, confirmText, confirmColor) => {
        setConfirmModal({
            isOpen: true,
            id: property._id?.$oid || property._id || property.id,
            title: property.title || 'Untitled',
            action: action,
            message: message,
            confirmText: confirmText,
    ...
    const handleConfirmAction = async () => {
        ...
        // If action is reject, open reject modal instead
        if (action === 'reject') {
            closeConfirmModal();
            openRejectModal(property);
            return;
        ...
        return (
            <div className="flex items-center justify-center min-h-[60vh] px-4">
                <div className="text-center bg-white dark:bg-gray-900 p-8 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 max-w-md">
                    ...
                    <Link href="/dashboard" className="mt-4 inline-block px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
                        Go to Dashboard
                    </Link>
                </div>
            </div>
            ...
            <div className="p-4 md:p-6">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                    <div>
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                            Total Properties: {pagination.totalItems}
                        </p>
                    </div>
                    <button
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        {property.images && property.images[0] ? (
                                                            <img
                                                                ...
                                                                onError={(e) => {
                                                                    e.target.src = 'https://via.placeholder.com/48x48/CCCCCC/FFFFFF?text=No+Image';
                                                                }}
                                                            />
                                                        ) : (
                                                            <div className="w-12 h-12 rounded-lg bg-gray-200 dark:bg-gray-800 flex items-center justify-center">
